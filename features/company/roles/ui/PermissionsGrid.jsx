    ...
    <div className="space-y-6">
      ...
      {warnings.length > 0 && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-3 space-y-1">
          ...
          {warnings.map((w) => (
            <p
              key={`${w.requiredBy}-${w.permission}`}
              className="text-xs text-amber-700 ml-6"
            >
              <span className="font-medium">{w.requiredByLabel}</span> يتطلب{" "}
              <span className="font-medium">{w.label}</span> - سيتم منحها
              تلقائياً وفق قواعد الصلاحيات
            </p>
          ))}
        </div>
      )}
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              >
                ...
                <span className="ml-2 text-xs font-normal text-gray-500">
                  ({groupPerms.filter((p) => selected.includes(p.key)).length}/
                  {groupPerms.length})
                </span>
              </Label>
            </div>
