  ...
  if (selected.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-center text-sm text-gray-500">
        لم يتم تحديد أي صلاحيات. حدِّد الصلاحيات بالأعلى لمعاينة التأثير.
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50/50 p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-gray-900">معاينة الدور</h4>
        <span className="text-xs text-gray-500">
          {effectivePermissions.length} من أصل {ALL_PERMISSIONS.length} صلاحية
          {impliedOnly.length > 0 && (
            <span className="text-amber-600">
              {" "}
              ({impliedOnly.length} مُنحت تلقائياً)
            </span>
          )}
        </span>
      </div>
