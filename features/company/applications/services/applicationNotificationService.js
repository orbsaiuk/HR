...
import { getOrganizationRoles } from "@/features/company/roles/services/rolesService";
import { getAllTeamMembers } from "@/features/company/org-members/services/orgMembersManagementService";
...
async function getTeamMembersWithPermission(orgId, permission) {
  ...
  return teamMembers
    .filter((tm) => roleKeysWithPermission.has(tm.roleKey) && tm.user?.email)
    .map((tm) => ({
      name: tm.user.name || "",
      email: tm.user.email,
    }));
...
export async function notifyNewApplication({
  ...
  try {
    ...
    const applicationUrl = `${appUrl}/company/positions`;
...
export async function notifyApplicationStatusChange({
  ...
  try {
    ...
    const organizationName =
      application.jobPosition?.organizationName || "the company";
