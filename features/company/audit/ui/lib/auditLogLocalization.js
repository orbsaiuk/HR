...
export function localizeAuditDescription(log) {
  ...
  if (action === "contract.created") {
    const title = extractQuotedValue(description) || log?.targetId || "";
    return title ? `تم إنشاء العقد "${title}"` : "تم إنشاء العقد";
  }

  if (action === "contract.whatsapp_sent") {
    const title = extractQuotedValue(description) || log?.targetId || "";
    return title ? `تم تجهيز إرسال واتساب للعقد "${title}"` : "تم تجهيز إرسال واتساب للعقد";
  }

  if (action === "contract.pdf_downloaded") {
    const title = extractQuotedValue(description) || log?.targetId || "";
    return title ? `تم تنزيل PDF للعقد "${title}"` : "تم تنزيل PDF للعقد";
  }

  if (action === "contract_template.created") {
    const title = extractQuotedValue(description) || log?.targetId || "";
    return title ? `تم إنشاء نموذج العقد "${title}"` : "تم إنشاء نموذج العقد";
  }
